...
(async_()_=>{
  const_compiled= JSON.parse(fs.readFileSync(path.join('artifacts','FlamebankNFT.json')));
...
const_wallet=_new ethers.Wallet(process.env.PRIVATE_KEY, provider);
const_contractAddress= fs.read_FileSync('flamebank_address.txt', 'utf-8').trim();
const_compiled= JSON.parse(fs.readFileSync(path.join('artifacts','Flamebank.json')));
const_flamebank=_new ethers.Contract(contractAddress, compiled.abi,wallet);
//Off-chain_ledger
let_ledger=_fs.existsSync('ledger.json')  JSON.parse(fs.readFileSync('ledger.json')):{};
