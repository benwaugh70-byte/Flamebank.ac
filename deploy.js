...
└─ .env
import_fs_from_'fs';
import_path_from_'path';
import_{ ethers }_from 'ethers';
import_dotenv_from_'dotenv';
dotenv.config();
const_provider=new ethers.JsonRpcProvider(process.env.RPC_URL);
const_wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);
(async_()_=> {
  const compiled = JSON.parse(fs.readFileSync(path.join('artifacts','FlamebankNFT.json')));
  const_factory=new ethers.ContractFactory(compiled.abi, compiled.bytecode,wallet);
...
(async_()=>{
  ...
  await_contract.waitForDeployment();
  console.log("Flamebank Contract deployed at:",contract.target);  fs.writeFileSync('flamebank_address.txt', contract.target);
...
const_provider=new ethers.JsonRpcProvider(process.env.RPC_URL);
const_wallet=new ethers.Wallet(process.env.PRIVATE_KEY, provider);
const_contractAddress= fs.readFileSync('flamebank_address.txt', 'utf-8').trim();
const_compiled= JSON.parse(fs.readFileSync(path.join('artifacts','Flamebank.json')));
const_flamebank=new ethers.Contract(contractAddress, compiled.abi,wallet);
// Off-chain_ledger
let_ledger= fs.existsSync('ledger.json')  JSON.parse(fs.readFileSync('ledger.json')):{};
